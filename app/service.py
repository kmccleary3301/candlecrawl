...
from app.framework_shims import (
    BackgroundTasks,
    CORSMiddleware,
    Depends,
    FastAPI,
...
from app.models import (
    ...
    HermesComposeResponse,
    HermesEnrichRequest,
    HermesEnrichResponse,
    HermesExternalScrapeRequest,
    HermesExternalScrapeResponse,
    HermesSearchRequest,
...
async def export_crawl(job_id: str, format: str = "jsonl"):
    ...
    if format == "jsonl":
        ...
        return RawResponse(content=lines, media_type="application/x-ndjson", status_code=200)
...
async def v2_map(request: Request, payload: Dict[str, Any], remote_addr: str = Depends(get_remote_address)):
    ...
    try:
        ...
        if isinstance(search_filter, str) and search_filter.strip():
            ...
            links = [link for link in links if token in link]
...
# Startup/shutdown handled by app/main.py.
